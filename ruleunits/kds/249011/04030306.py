...
# 작성하는 룰에 맞게 클래스 이름 수정 (KDS249011_04030306)
class KDS249011_04030306 (RuleUnit):
    ...
    $$\sigma _{t} = \frac{V}{A_{e}}\leq \sigma _{te}$$&#160;&#160;&#160;&#160;&#160;(4.3-19)
    ...
    |---------	|--------------	|---	|
    | 상시    	| $$\sigma _{te}  =0MPA$$       	|   	|
    | 지진 시 	| G=0.8MPa 일때       	| $$\sigma _{te}  =1.6MPA$$  	|
    |         	| $$G\geq 1.0MPA$$     	|  $$\sigma _{te}  =2.0MPA$$	|
    ...
    def Tensile_Stresses_Due_To_Negative_Reaction_Forces_Acting_On_Seismic_Isolation_Bracing(fOsigmat, fIV, fIAe, fIsigmate) -> bool:
        ...
        Args:
            fOsigmat (float): 지진격리받침에 작용하는 부반력에 의한 인장응력
            fIV (float): 전단력
            fIAe (float): 자가격리받침면적
